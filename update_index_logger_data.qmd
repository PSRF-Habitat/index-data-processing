---
title: "Update Index Logger Data Main File"
format: html
embed-resources: true
toc: true
editor: 
  markdown: 
    wrap: 72
editor_options: 
  chunk_output_type: console
---

# Overview

Use this script to update the main index site logger data file when new
data becomes available.

# Requirements

## Drive Organization and File Naming

Folder structure in Drive must match this format:

```         
.
└── csvfiles/
    ├── Temperatue/
    │   ├── SiteName_LoggerNickname_Deployment-date.csv
    │   └── ex: SquaxinIsland_T4_2024-04-20.csv
    ├── pH/
    │   ├── SiteName_LoggerNickname_Deployment-date.csv
    │   └── ex: Magnolia_PH11_2024-04-20.csv
    ├── Water Level/
    │   ├── SiteName_LoggerNickname_Deployment-date.csv
    │   └── ex: PointVashon_WL3_2024-04-20.csv
    └── ...etc
```

Here, the root folder is `csvfiles/`, with sub-folders corresponding to
the sensor types. While the name of the root folder can be changed, it
is important that the sub files are named `Temperature`, `pH`,
`Water Level`, `Dissolved Oxygen`, `Conductivity`, and `PAR` (case
insensitive).

Within each sub-folder, each file to be processed needs to follow the
naming convention: `SiteName_LoggerNickname_Deployment-date.csv`

Where:

-   `SiteName` is the full site name with each new word capitalized

-   `LoggerNickname` is the nickname given to that specific instrument
    in the metadata sheet

-   `Deployment-date` in the format yyyy-mm-dd is EITHER:

    -   Date deployed or

    -   Date re-launched (whichever corresponds to the start of true
        data in that file)

-   File needs to end in `.csv`, else it will NOT be picked up when searching the Drive

-   Each part is separated by an underscore `_`

## Metadata sheet

It is important that the `SiteName`, `LoggerNickname`, and
`Deployment-date` are found in the metadata. This is how the code will
find more information about each file! Ensure that these 3 parts are
found in metadata columns `Site`, `Nickname`, and either
`Initial Deployment Date` or `RelaunchDate`, respectively.

# Update the Main Data File

## Step 1: Setup

First we will load the necessary packages for processing. Run this code
chunk before taking any other steps (you may need to install packages
using `install.packages("package-name-here")` if you haven't done so
already):

```{r}
# For interacting directly with Google Drive
library(googledrive)
library(googlesheets4)

# For data cleaning and manipulation
library(tidyverse)
library(janitor)
library(lubridate)
library(here)

# We also need to source our functions
source(here("functions", "index_data_processing_functions.r"))
```

If it is your **first time** ever connecting to the Google Drive API, you will
need to authenticate your account. Run these two lines one by one and
follow the instructions in your Console:

```{r}
# Authenticate with Google Drive (only need to do this once)
# drive_auth()
# gs4_auth()
```

## Step 2: Specify Parameters

Identify which folder and metadata sheet you want to process. I have pre-filled each parameter with the folder and file IDs as they currently are in the drive. If these folders or files are ever relocated, you will need to change these parameters.

Un-comment whichever group you would like to process. 

Replace the values in the quotation marks `" "` if necessary.

```{r}
######### PSRF Sites ##########
# Folder to search in for logger files. Currently named "csvfiles" in Drive
# Folder ID found after the final / in the folder's URL
# root_folder_id = "1k5u8iOhR5alnymc7BVU-JJjcSnQStgWv"

# # FOR MARINA TESTING
# root_folder_id = "1TarMgY-SC4L-iTCyeuo-6UVwP9Gaj6U0"
## MARINA TESTING EVEN SMALLER
root_folder_id = "1HYfpizZ_6jt-9hjKtPPsnwBz4lQM0JQQ"

# Link to the metadata sheet
metadata_file_url = "https://docs.google.com/spreadsheets/d/1JJ4Vtb_pI9FJRvMYg7pbhQzM6JPcFzGVU3rR9IdYnGQ/edit?gid=1334564098#gid=1334564098"

# Name of the tab with the file metadata
sheet_name = "Reorganized_Deployments_IndexOnly"


# ######### Reef Check Sites ##########
# # Folder to search in for logger files. Currently named "CSV" in Drive
# # Folder ID found after the final / in the folder's URL
# root_folder_id = "1fvtprLxnCWSH3EQ3DUC4cyvgUGvPBGeN"
# 
# # Full link to the metadata sheet
# metadata_file_url = "https://docs.google.com/spreadsheets/d/1-p-RWjFonTMefs3JIqPIOhIjcWqFwmQ9sE6ty4WEkOg/edit?gid=0#gid=0"
# 
# # Name of the tab with the file metadata
# sheet_name = "Deployments"

# ########## Point Defiance Zoo & Aquarium Sites ##########
# # Folder to search in for logger files. Currently named "CSV" in Drive
# # Folder ID found after the final / in the folder's URL
# root_folder_id = "17jpThnU1QRwp3Tq09XacaQS9fQKnYxRv"
# 
# # Full link to the metadata sheet
# metadata_file_url = "https://docs.google.com/spreadsheets/d/1QBCwXV7_KCdYab-zWmXJXDDACZXpJVtLNQuzltwy3fs/edit?gid=0#gid=0"
# 
# # Name of the tab with the file metadata
# sheet_name = "Deployments"


# ######### Friday Harbor Labs Sites ##########
# # Folder to search in for logger files. Currently named "CSV" in Drive
# # Folder ID found after the final / in the folder's URL
# root_folder_id = "1ZR9EpXXhPtk-9R6eyDI-wzs-m0My36c0"
# 
# # Full link to the metadata sheet
# metadata_file_url = "https://docs.google.com/spreadsheets/d/1xQ_MkgwL1-ewZWO1tq8LaTLUHW-YOx2nVj_cQ7sz4pw/edit?gid=0#gid=0"
# 
# # Name of the tab with the file metadata
# sheet_name = "Deployments"
```

## Step 3: Pre-run Check

Before running the full pipeline, run this chunk to check that all files in the Drive folder have a matching entry in the metadata sheet. Files without a match will be skipped during the next processing stage, so catching them here saves you from running the full pipeline only to find files were missed.

This chunk will also check for metadata entries with missing in water or out of water dates, which are important to fill in, else be silently filtered out of the final dataset.

```{r}
# Run metadata & file name check
checks <- prerun_check(root_folder_id = root_folder_id,
                metadata_file_url = metadata_file_url,
                sheet_name = sheet_name)

##### MAIN CHECK #####
# See files that will not be processed
View(checks$unmatched)

##### Optional Checks #####
# --- may assist in finding common metadata mistakes --- #
# See metadata rows with invalid dates
# View(checks$invalid_dates)

# See duplicate metadata rows
# View(checks$duplicate_rows)
```

If any files are flagged, fix their entries in the metadata sheet in Google Sheets before
proceeding to Step 4. Common issues to check:

- Site name spelling matches between the file name and metadata
- Logger nickname in file name matches the `Nickname` column in metadata     
  exactly
- Deployment date in the file name matches either `Initial Deployment Date` 
  or `Relaunch Date` in metadata
  
Please also check that all metadata entries that have an associated data file have both an in water and out of water date.

After fixing issues in file names and metadata in Google Drive, re-run the check. Once the pre-run check returns no issues, proceed to Step 4!


## Step 4: Update File

Use the next chunk of code to update the kelp index site's logger data
from the Google Drive. Choose how you would like to update the data,
highlight that code with your mouse and un-comment (remove the `#`)
using `command + shift + c` on mac or `ctrl + shift + c` on PC, then run
the line by placing your cursor after the closing parenthesis `)` and
pushing `command + return` on mac or `ctrl + enter` on pc.

```{r}
# ########## Add new files to existing file ##########
# 
# data <- update_logger_data_incremental(root_folder_id = root_folder_id,
#                                         metadata_file_url = metadata_file_url,
#                                         sheet_name = sheet_name)



########## Re-process all old files in addition to new files  ##########

data <- update_logger_data_incremental(root_folder_id = root_folder_id,
                                       metadata_file_url = metadata_file_url,
                                       sheet_name = sheet_name,
                                       force_reprocess = TRUE,
                                       verbose = FALSE)
```

## Step 5: Export as CSV to Google Drive

```{r}
# ===== Logger Data ===== #
today <- Sys.Date()

# Full descriptive names for filenames
sensor_names <- c(
  temp = "temperature",
  ph   = "ph",
  do   = "dissolved_oxygen",
  wl   = "water_level",
  par  = "par",
  con  = "conductivity"
)

# Loop through each sensor type in the list and upload separately
for (sensor_type in names(data)) {
  
  df <- data[[sensor_type]]
  
  # Use the full name
  full_name <- ifelse(sensor_type %in% names(sensor_names),
                       sensor_names[[sensor_type]],
                       sensor_type)
  
  csv_filename <- paste0("kelp_index_", full_name, "_sensor_data_", today, ".csv")
  
  temp_csv <- tempfile(fileext = ".csv")
  write_csv(df, temp_csv)
  
  drive_upload(media = temp_csv,
               name = csv_filename,
               path = as_id("1uGfyiyj-xAdy6xM2j81l7jDZK9SgNBw6"),
               overwrite = TRUE)
  
  unlink(temp_csv)
}


# ===== List of processed files ===== #
processed_filename <- paste0("processed_files_", today, ".csv")
processed_files <- read_rds(here("data", "processed_files.rds"))
processed_files <- as.data.frame(processed_files)

temp_csv <- tempfile(fileext = ".csv")
write_csv(processed_files, temp_csv)

drive_upload(media = temp_csv,
             name = processed_filename,
             path = as_id("1uGfyiyj-xAdy6xM2j81l7jDZK9SgNBw6"),
             overwrite = TRUE)

unlink(temp_csv)
```

# Data Review

Use this section to review data and issue flags for either the entire dataset or the most recently added deployments. Use these plots to help inform what data should be filtered out (e.g., points crash to 0 or issue flag notes that a surface instrument was recovered from the bottom). 

## SST

Surface temperature from the tidbit loggers

```{r}
# Build plots
sst_plots <- plot_issue_flags(data$temp, "tidbit_temp_c", "Temperature (°C)", "Temperature")

# Scroll through all plots in plot viewer
walk(sst_plots[!sapply(sst_plots, is.null)], print)

# Or refer to by name
sst_plots[["Centennial Park - surface"]]
```

## Temp - bottom

need to figure out a way to only show bottom for this one!!!!

```{r}
# Build plots
temp_bottom_plots <- plot_issue_flags(data$ph, "ph_temp_c", "Temperature (°C)", "Temperature")

# Scroll through all plots in plot viewer
walk(temp_bottom_plots[!sapply(temp_bottom_plots, is.null)], print)

# Or refer to by name
temp_bottom_plots[["Jefferson Head - bottom"]]
```


## pH

```{r}
# Build plots
ph_plots <- plot_issue_flags(data$ph, "pH", "pH", "pH")

# Scroll through all plots in plot viewer
walk(ph_plots[!sapply(ph_plots, is.null)], print)

# Or refer to by name
ph_plots[["Jefferson Head - bottom"]]
```

## Dissolved Oxygen

```{r}
# Build plots
do_plots <- plot_issue_flags(data$do, "do_conc_mg_per_L", "DO", "Dissolved Oxygen")

# Scroll through all plots in plot viewer
walk(do_plots[!sapply(do_plots, is.null)], print)

# Or refer to by name
do_plots[["Jefferson Head - bottom"]]
```


## Conductivity

```{r}
# Build plots
con_plots <- plot_issue_flags(data$con, "high_range_microsiemens_per_cm", 
                              "CON", "Conductivity")

# Scroll through all plots in plot viewer
walk(con_plots[!sapply(con_plots, is.null)], print)

# Or refer to by name
con_plots[["Jefferson Head - bottom"]]
```

## Water Level

```{r}
# Build plots
wl_plots <- plot_issue_flags(data$wl, "abs_pres_kpa", 
                              "Absolute Pressure", "Water Level")

# Scroll through all plots in plot viewer
walk(wl_plots[!sapply(wl_plots, is.null)], print)

# Or refer to by name
wl_plots[["Jefferson Head - bottom"]]
```

## PAR

```{r}
# Build plots
par_plots <- plot_issue_flags(data$par, "calibrated_integrating_light", 
                              "Light", "PAR")

# Scroll through all plots in plot viewer
walk(par_plots[!sapply(par_plots, is.null)], print)

# Or refer to by name
par_plots[["Jefferson Head - bottom"]]
```



# Visual Check

The final, optional step is to visually check the data.

#### Tidbit Temperature

Surface:

```{r}
#| code-fold: true
# Surface temperature by site
data$temp |>
  filter(position == "surface") |>
  ggplot() +
  geom_point(aes(x = datetime, y = tidbit_temp_c), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "SST by Site",
       x = "Date",
       y = "Temperature (°C)") +
  theme_minimal()

```

#### pH

Surface:

```{r}
#| code-fold: true
# pH surface
data$ph |>
  filter(position == "surface") |>
  ggplot() +
  geom_point(aes(x = datetime, y = pH), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Surface pH by Site",
       x = "Date",
       y = "pH") +
  theme_minimal()
```

Bottom:

```{r}
#| code-fold: true
# pH bottom
data$ph |>
  filter(position == "bottom") |>
  ggplot() +
  geom_point(aes(x = datetime, y = pH), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "pH at Bottom by Site",
       x = "Date",
       y = "pH") +
  theme_minimal()

```

#### Dissolved Oxygen

Surface:

```{r}
#| code-fold: true
# DO surface
# Raw
data$do |>
  filter(position == "surface") |>
  ggplot() +
  geom_point(aes(x = datetime, y = do_conc_mg_per_L), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Dissolved Oxygen at Surface by Site",
       x = "Date",
       y = "Dissolved Oxygen (mg/L)") +
  theme_minimal()


# No values below zero
data$do |>
  filter(position == "surface") |>
  filter(do_conc_mg_per_L >= 0) |>
  ggplot() +
  geom_point(aes(x = datetime, y = do_conc_mg_per_L), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Dissolved Oxygen at Surface by Site (Filtered to >= 0)",
       x = "Date",
       y = "Dissolved Oxygen (mg/L)") +
  theme_minimal()
```

Bottom:

```{r}
#| code-fold: true
# DO bottom
# Raw
data$do |>
  filter(position == "bottom") |>
  ggplot() +
  geom_point(aes(x = datetime, y = do_conc_mg_per_L), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Dissolved Oxygen at Bottom by Site",
       x = "Date",
       y = "Dissolved Oxygen (mg/L)") +
  theme_minimal()

# No values below zero
data$do |>
  filter(position == "bottom") |>
  filter(do_conc_mg_per_L >= 0) |>
  ggplot() +
  geom_point(aes(x = datetime, y = do_conc_mg_per_L, color = logger_id), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Dissolved Oxygen at Bottom by Site (Filtered to >= 0)",
       x = "Date",
       y = "Dissolved Oxygen (mg/L)") +
  theme_minimal()

```

#### Conductivity

Surface:

```{r}
#| code-fold: true
# CON surface
data$con |>
  filter(position == "surface") |>
  ggplot() +
  geom_point(aes(x = datetime, y = high_range_microsiemens_per_cm), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Conductivity at Surface by Site",
       x = "Date",
       y = "Conductivity (µS/cm)") +
  theme_minimal()

```

Bottom:

```{r}
#| code-fold: true
# CON bottom
data$con |>
  filter(position == "bottom") |>
  ggplot() +
  geom_point(aes(x = datetime, y = high_range_microsiemens_per_cm), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Conductivity at Bottom by Site",
       x = "Date",
       y = "Conductivity (µS/cm)") +
  theme_minimal()

```

#### Water Level

Bottom:

```{r}
#| code-fold: true
# WL bottom
data$wl |>
  filter(position == "bottom") |>
  ggplot() +
  geom_point(aes(x = datetime, y = abs_pres_kpa), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "Water Level at Bottom by Site",
       x = "Date",
       y = "Absolute Pressure (kpa)") +
  theme_minimal()

```

#### Light Intensity (PAR)

Bottom:

```{r}
#| code-fold: true
# PAR bottom (raw)
data$par |>
  filter(position == "bottom") |>
  ggplot() +
  geom_point(aes(x = datetime, y = raw_integrating_light), size = 0.1) +
  facet_wrap(~site) +
  labs(title = "PAR Raw at Bottom by Site",
       x = "Date",
       y = "Raw Integrating Light") +
  theme_minimal()

```


