...
read_and_clean_logger_csv <- function(file_row, metadata, verbose = FALSE) {
  ...
  ##### CONDUCTIVITY LOGGER #####
    ...
    # Set NA for days prior to deployment and after retrieval. 
    # Note: keeping deployment days as non-NA values and noting them in a new column date_flag
    # This is so we can later groundtruth & calibrate con to cast data
    df <- df |>
      mutate(
        date = as.Date(datetime),
...
#' Find the DST "fall-back" Sunday for a given year ----
#'
...
#' non-NA value per column. The exception is rows that fall in the November 
#' "fall-back hour" (1:00-1:59 AM) where we see distinct readings with matching timestamps. 
#' The second reading's datetime is nudged forward by a few seconds so it stays unique, 
#' and a note is stashed in a temporary `dst_fallback_note` column so it can be added
#' to `comments` in the final dataset
#'
...
update_logger_data_incremental <- function(root_folder_id, metadata_file_url, sheet_name,
  ...
  if (!force_reprocess) {
    for (i in seq_along(sensor_types)) {
      ...
      if (file.exists(data_file_path)) {
        cli::cli_alert_info(paste("Loading existing", sensor_names[i], "data..."))
        existing_data[[sensor_types[i]]] <- readRDS(data_file_path)
        cli::cli_alert_success(paste("Existing", sensor_names[i], "data has", 
  ...
  if (file.exists(tracking_file_path) && !force_reprocess) {
    ...
    cli::cli_alert_info(paste("Found", length(processed_files), "previously processed files"))
  }
  
  # Get current file list from Drive
  cli::cli_alert_info("Scanning Google Drive for files...")
  all_files <- get_all_logger_csvs_by_id(root_folder_id)
  
  if (nrow(all_files) == 0) {
    cli::cli_alert_warning("No CSV files found in Drive folder")
  ...
  if (force_reprocess) {
    ...
    cli::cli_alert_warning(paste("Force reprocess: will process all", nrow(files_to_process), "files"))
  } else {
    files_to_process <- all_files |>
      filter(!(name %in% processed_files))  # Only process files whos name is not in file tracker
    cli::cli_alert_info(paste("Found", nrow(files_to_process), "new files to process"))
  }
  
  # If no new files, return existing data
  if (nrow(files_to_process) == 0) {
    cli::cli_alert_success("No new files to process, data is up to date.")
    return(existing_data)
  }
  
  # Load metadata (always refresh this in case of updates)
  cli::cli_alert_info("Loading metadata...")
  metadata <- read_and_clean_metadata(metadata_file_url, sheet_name)
  
  # ===== 2. Process new files ===== #
  cli::cli_h2("Processing new files")
  cli::cli_alert_info(paste("Processing", nrow(files_to_process), "new files..."))
  ...
  for (i in seq_len(nrow(files_to_process))) {
    ...
    cli::cli_alert_info(paste("Processing new file", i, "of", nrow(files_to_process), ":", file_row$name))
    ...
    tryCatch({ # trying this new way of throwing warnings/errors
      ...
      if (is.null(cleaned_data)) {
        cli::cli_alert_warning(paste("Skipping file (no metadata match):", file_row$name,
        ...
        cli::cli_alert_success(paste("Successfully processed:", file_row$name))
      }
      
    }, error = function(e) {
      cli::cli_alert_danger(paste("Error processing file", file_row$name, ":", e$message))
      cli::cli_alert_info("Check that file name and metadata are as expected")
      cli::cli_alert_info(paste("File", file_row$name, "will be retried on next run"))
      unsuccessfully_processed_files <<- c(unsuccessfully_processed_files, file_row$name)
    })
  }
  ...
  cli::cli_h2("Completing sensor datasets")
  ...
  for (i in seq_along(sensor_types)) {
    ...
    if (is.null(new_data) && is.null(old_data)) {
      cli::cli_alert_info(paste("No", sensor_name, "data to process"))
      final_data[[sensor]] <- NULL
      next
    }
    
    cli::cli_alert_info(paste("Processing", sensor_name, "data..."))
    ...
    # disticnt deployments, used to more efficiently complete timeseries and add in 
    # final issue_flag/comments
    ...
    # Make sure the completed 15-minute time series always spans
    ...
    # and flagging DST fall-back duplicates
    ...
    # Assign issue_flag and comments from metadata for the whole deployment window
    ...
    combined_data <- combined_data |>
      ...
      # Append the DST fall-back note onto whatever comment already exists
      mutate(
        comments = case_when(
    ...
    cli::cli_alert_success(paste(sensor_name, "data:", nrow(combined_data), "rows"))
    final_data[[sensor]] <- combined_data
  }
  
  
  # 4. Save updated data and tracking info
  cli::cli_h2("Saving updated data")
  ...
  for (sensor in sensor_types) {
    if (!is.null(final_data[[sensor]])) {
      ...
      cli::cli_alert_success(paste("Saved", sensor_names[which(sensor_types == sensor)], 
  ...
  cli::cli_alert_success(paste("Processing complete!"))
  cli::cli_alert_success(paste("Successfully processed", length(successfully_processed_files), "new files"))
  cli::cli_alert_warning(paste("Failed to process", nrow(files_to_process) - length(successfully_processed_files), "files"))
  ...
  for (i in seq_along(sensor_types)) {
    if (!is.null(final_data[[sensor_types[i]]])) {
      cli::cli_alert_info(paste(sensor_names[i], "dataset:", 
