...
Going for one chart with site on the y and date on the x, with Gantt-style bars when we have any available data for that site.


```{r}
data_availability <- data |>
  mutate(has_data = if_any(c("raw_integrating_light",
                             "abs_pres_kpa",
                             "high_range_microsiemens_per_cm",
                             "do_conc_mg_per_L",
                             "millivolts",
                             "tidbit_temp_c"), function(x) !is.na(x))) |>
  group_by(site) |>
  arrange(datetime) |>
  mutate(data_group = cumsum(has_data != lag(has_data, default = first(has_data)))) |>
  filter(has_data) |>
  group_by(site, data_group) |>
  summarize(start_date = min(datetime),
            end_date = max(datetime),
            .groups = "drop") |>
  filter(site != "office",
         site != "manchesterref") |>
  mutate(site = fct_reorder(site, start_date, .fun = min, .desc = TRUE))
...
site_labels <- c(
  ...
  "northwestnarrows" = "Northwest Narrows",
  "foulweatherbluff" = "Foulweather Bluff",
  "freshwaterbay" = "Freshwater Bay",
  "magnolia" = "Magnolia",
  "lincolnpark" = "Lincoln Park",
  "pointvashon" = "Point Vashon",
  "pointcaution" = "Point Caution"
)

# plot 
p_all <- ggplot(data_availability) +
  ...
  geom_segment(aes(x = start_date, xend = end_date,
                   y = site, yend = site), linewidth = 4) +
  scale_y_discrete(labels = site_labels) +
  scale_x_datetime(labels = label_date("%b"),
                   breaks = date_breaks("1 month"),
                   limits = as.POSIXct(c("2023-01-01", "2025-12-1")),
                   expand = c(0.02, 0)
  ) +
  labs(title = "Sensor Data Availability Across Kelp Index Sites",
       subtitle = "Horizontal bars show periods with available data") +
  theme_minimal() +
  theme(
...
ggsave(here("figures", "all_timeline.pdf"), p_all, width = 10, height = 6)
```
